...
//this method is for close the modal 
  ...
  UpdateRating(key) {
      ...
      setTimeout(function(){this.setState({modalVisible: false})}.bind(this), 2000);
      // navigation to app store and play store 
      if (Platform.OS == 'ios') {
        var browseurl = 'https://itunes.apple.com/us/app/maxword/id1071412401?mt=8'
        console.log('appstore:')
      } else {
        var browseurl = 'https://play.google.com/store/apps/details?id=com.maxwords&hl=en_US'
      }
      return Linking.openURL(browseurl)
    }
    
    RateNow(){
        setTimeout(function(){this.setState({modalVisible: false})}.bind(this), 100);
         // navigation to app store and play store 
        if (Platform.OS == 'ios') {
            ...
            <View style={{height:50,marginRight:25,marginLeft:25,}}>
              <View style={{flex:1,flexDirection:'row',}}>
               {this.state.hide=='true' ?(
                     <TouchableOpacity style={styles.rateStyle}>
                      <View style={styles.container} >
                        <Text allowFontScaling={false} style={styles.buttonTextStyle}>Rate Now</Text>
                      </View>
                    </TouchableOpacity>
                ):
                <TouchableOpacity style={styles.rateStyle} 
              ...
              {this.state.hide =='true' ?(
                <TouchableOpacity style={styles.askLaterStyle}>
                  <View style={styles.container}>
                    <Text allowFontScaling={false} style={styles.buttonTextStyle}>Ask Later</Text>
                  </View>
                </TouchableOpacity>
             ):
                 <TouchableOpacity style={styles.askLaterStyle}
